...
const editAddress = async (req, res) => {
    try {
        ...
        const { fullName, phone, houseName, street, city, district, state, pincode, landmark, addressType } = req.body;
        const formData = { fullName, phone, houseName, street, city, district, state, pincode, landmark, addressType };
        const errors = validateAddressFields(formData);
        if (Object.keys(errors).length > 0) {
            return res.status(400).json({
                success: false,
                message: 'Please correct the errors below',
        ...
        await Address.findByIdAndUpdate(addressId, formData);
...
const deleteAddress = async (req, res) => {
    try {
        ...
        if (!address) return res.status(404).json({ success: false, message: 'Address not found or already deleted.' });
